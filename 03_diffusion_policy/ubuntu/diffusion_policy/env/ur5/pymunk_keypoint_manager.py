...
class PymunkKeypointManager:
    ...
    @classmethod
    def create_from_ur5_env(cls, env, n_block_kps=9, n_agent_kps=3, seed=0, **kwargs):
...
def test():
    from diffusion_policy.environment.ur5_env import ur5Env
    from matplotlib import pyplot as plt
    
    env = ur5Env(headless=True, obs_state=False, draw_action=False)
    kp_manager = PymunkKeypointManager.create_from_ur5_env(env=env)
    env.reset()
    obj_map = {
        'block': env.block,
        'agent': env.agent
